...
class BrowserKeeper:
	...
	def start(self):
		...
		try:
			...
			# Enable background process priority
			self.webdriver.execute_cdp_cmd('Emulation.setFocusEmulation', {'focused': True})
			self.webdriver.execute_cdp_cmd('Page.setWebLifecycleState', {'state': 'active'})
	...
	def _keep_alive_loop(self):
		...
		while not self._stop_event.is_set() and error_count < max_errors:
			try:
				# Send CDP commands to keep browser active
				self.webdriver.execute_cdp_cmd('Page.setWebLifecycleState', {'state': 'active'})
